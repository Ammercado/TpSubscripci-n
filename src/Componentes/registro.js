  ...
  comprobar(){
    ...
    if((this.state.nombre.length > 0) &&
     ...
     if(this.state.activeStep === 1 && this.state.tipo === "premiun"){
       if((this.state.tarjetaNum.length >= 16) &&
     (this.state.tarjetaExp.length >= 4) &&
     (this.state.tarjetaCvv.length >= 3)){
     }else{
       verificar = false;
     }
     }
          ...
          if(this.state.tipo ==='free'){
            ...
            return(
              <div>
                ...
                <CreditCardInput
                  cardNumberInputProps={{ minLength:16, pattern:"[0-9]*", value: this.state.handletarjetaNumberChange, onChange: this.handletarjetaNumberChange }}
